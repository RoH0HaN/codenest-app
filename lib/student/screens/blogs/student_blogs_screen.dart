...
class StudentBlogsScreen extends StatelessWidget {
  const StudentBlogsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<StudentBlogsController>();

    return Scaffold(
      appBar: AppBar(
        leading: _buildBackButton(context),
        title: Text(
          AppTexts.exploreBlogs,
  ...
  Widget _buildCategoryFilters(
      BuildContext context, StudentBlogsController controller) {
    return Obx(() => Wrap(
          spacing: 10,
          runSpacing: 5,
